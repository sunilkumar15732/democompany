...
if ( ! function_exists( 'psd2wp_setup' ) ) :
	...
	function psd2wp_setup() {
		...
		register_nav_menus( array(
			...
			'menu-2' => esc_html__( 'secondary', 'psd2wp' ),
			'menu-3' => esc_html__( 'widget-menu', 'psd2wp' ),
		) );
...
function psd2wp_scripts() {
	...
	wp_enqueue_script( 'psd2wp-bootstrap-js', get_template_directory_uri() . '/bootstrap/js/bootstrap.min.js', array(), '20151215', true  );
	...
	wp_enqueue_script( 'psd2wp-slippry-js', get_template_directory_uri() . '/slider/slippry.min.js', array(), '', FALSE  );

	wp_enqueue_style( 'psd2wp-slippry-css', get_template_directory_uri() . '/slider/slippry.css' );
...
function learning_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => __( 'footer copyright text', 'theme_name' ),
        'id'            => 'footer-text',
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget'  => '</aside>',
        'before_title'  => '<h1 class="widget-title">',
 ...
 * Custom latest post
 */
require get_template_directory() . '/inc/latest.php';
